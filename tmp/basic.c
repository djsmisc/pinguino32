/*	----------------------------------------------------------------------------
	ATIBI : A Tiny Basic Interpreter
	----------------------------------------------------------------------------
...
void run(), load_program();
void basic_print(), scan_labels(), find_eol(), exec_goto();
...
run(int argc, char *argv[])
{
	...
	if(argc!=2)
	{
		Serial.printf("usage: run <filename>\n");
		exit(1);
	}
	...
	{
		Serial.printf("allocation failure");
		exit(1);
	}
...
void input()
{
	...
	else Serial.printf("? "); /* otherwise, prompt with / */
