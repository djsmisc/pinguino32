...
/*	---Use one of these forms------------------------------------------------------------
	Serial.Configure(	UART1, 0x8000, 0x1400, 9600);
	Serial.Configure(	UART1, UART_ENABLE,	UART_RX_TX_ENABLED,	baudrate);
	Serial.Configure(	UART1,
							...
							baudrate);
/*	---or-----------------------------------------------------------------------
	Serial.PinConfigure(UART1);
	Serial.SetDataRate(UART1, baudrate);
	Serial.SetLineControl(UART1, UART_ENABLE | UART_ENABLE_PINS_TX_RX_ONLY | UART_8_BITS_NO_PARITY | UART_STOP_BITS_1);
	Serial.Enable(UART1, UART_RX_TX_ENABLED | UART_INTERRUPT_ON_RX_FULL);
	Serial.IntConfigure(UART1, INT_PRIORITY_3, INT_SUBPRIORITY_3);
	--------------------------------------------------------------------------*/
...
/*	---Test---------------------------------------------------------------------
	...
	loop
	{
		Serial.Printf(UART1, "Decimal[%5d] = Hexa[0x%4X] %s\n\r", i, i, string);
		i++;
	}
	--------------------------------------------------------------------------*/
}
