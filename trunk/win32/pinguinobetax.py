...
# ------------------------------------------------------------------------------
    ...
    def OnVerify(self, event):
        ...
        if self.board_menu.IsChecked(self.ID_P32X):
            if os.path.exists(self.Pinguino32XFolder+"\\bin\\"+self.c32name):
        ...
        if self.editor.GetPath()!=-1:
            ...
            if os.path.exists(filename+".hex"):
                ...
                if self.board_menu.IsChecked(self.ID_P32X):
                    ...
                    sortie=Popen([sys.path[0]+"\\tools\\bin\\ubw32",
                                "-w",
                                filename+".hex",
                                "-r",
                                "-n"],
    ...
    def compile32(self,filename):
        ...
        sortie = Popen([self.Pinguino32XFolder + "\\bin\\"+self.c32name,
                    ...
                    "-D "+self.devtool,
                    "-I" + sys.path[0] + "\\tools\\pic32mx\\include\\pinguino\\",
                    "-I" + sys.path[0] + "\\tools\\pic32mx\\include\\",
                    "-I" + chemin + "\\",
                    "-x",
                    "c",
                    "-c",
                    "-o" + sys.path[0] + "\\source\\main32.o",
                    sys.path[0] + "\\source\\main32.c"],
                    stdout=fichier, stderr=STDOUT)
        sortie.communicate()
        fichier.seek(0)
        if sortie.poll()!=0:
