...
*/
	...
	RTCC.setAlarmRepeatCount(10);			// repeat alarm 10 times = 10 interrupt events
													...
													// depends on the alarmRepeat used.
...
/**	-----------------------------------------------------------------------------
	...
	---------------------------------------------------------------------------*/
	CDC.printf("Alarm is ");
	CDC.printf("%03s, %02d %03s %04d, ", Day[alDate.wday], alDate.mday, Month[alDate.mon], alDate.year+2000);
	CDC.printf("%02d:%02d:%02d  ", alTime.hour, alTime.min, alTime.sec);
/**	-----------------------------------------------------------------------------
	alarm output management
	---------------------------------------------------------------------------*/
	status = digitalRead(RTCC);	// RTCC pin
	//status = digitalRead(RTCCpin);	// RTCC pin
