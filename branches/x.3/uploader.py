...
# Device Descriptor : lsusb -v -d 04d8:feaa
	...
	def initDevice(self, device, board):
		...
		if handle:
			...
			if board.bldr == 'vasco':
				...
				handle.setConfiguration(self.MCC_ACTIVE_CONFIG)
...
# ------------------------------------------------------------------------------
	def usbWrite(self, handle, usbBuf):
		...
		if not handle.bulkWrite(self.OUT_EP, usbBuf, self.TIMEOUT):
...
# ------------------------------------------------------------------------------
	def hexWrite(self, output, handle, filename, board):
		...
		if board.bldr == 'vasco':
			...
			blocksize = 64 - self.DLN_BOOT_CMD_SIZE
		else:
			blocksize = 64
			
		percent = (max_address - board.memstart) / 100
		...
		for i in range(board.memstart, max_address):
			...
			if i % blocksize == 0:
				index = 0
				if usbBuf != []:
					self.issueBlock(output, board, handle, i - blocksize, usbBuf)
				usbBuf = []
			if data[i] != []:
...
# ------------------------------------------------------------------------------
	def writeHex(self, output, filename, board):
		...
		if device is self.ERR_DEVICE_NOT_FOUND:
			...
			self.txtWrite(output, "Is your device in bootloader mode ?\n")
			return
		else:
		...
		if filename != '':
			...
			if status == self.ERR_NONE: self.txtWrite(output, os.path.basename(filename) + " successfully uploaded\n")
