...
void tick()
{
	...
	lcdi2c_setCursor(4, 1); // col 4, line 1
...
//
	lcdi2c_newchar(eacute, EACUTE);		// eacute is assigned to ascii 0
	lcdi2c_setCursor(2, 1);					// set cursor at line 1, col 2
	lcdi2c_write(EACUTE);					// replace 'e' by 'é'
	Delayms(5000);								// wait for 5 sec.
	lcdi2c_clearLine(1);
}
