...
typedef enum
{
    ...
    // Host has enabled (set) the remote wake up feature in the device.
    EVENT_SET_WAKE_UP,

    // Host has disabled (cleared) the remote wake up feature in the device.
    EVENT_CLEAR_WAKE_UP,
...
#define USBDisable()   USBHALDisable()  // Implemented as a macro
