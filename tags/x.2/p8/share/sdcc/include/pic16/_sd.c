/*	----------------------------------------------------------------------------
	...
	----------------------------------------------------------------------------
	TODO : make it compatible with Arduino syntax
	----------------------------------------------------------------------------
...
int SD_read_block(u16 block_number);
int SD_write_block(u16 block_number);
int SD_get_status();


/************************** SD Init **************************************/
/* Initialises the SD into SPI mode and sets block size, returns 0 on success */
...
int SD_init()
{
	...
	SPI_write(0x00);
	SPI_write(0x00);
	SPI_write(0x95);						// precalculated checksum as we are still in SD mode

	//puts("Sent go to SPI\n\r");

	if(SD_response(0x01)==1) return 1;		// if = 1 then there was a timeout waiting for 0x01 from the SD

	//puts("Got response from SD\n\r");

	i = 0;

	while((i < 255) && (SD_response(0x00)==1))// must keep sending command if response
	{
		SPI_write(0x41);					// send SD command one to bring out of idle state
	...
	if(i >= 254) return 1;					// if >= 254 then there was a timeout waiting for 0x00 from the SD

	//puts("Got out of idle response from SD\n\r");

	digitalwrite(SPI_SLAVE_SELECT, HIGH);	// set SS = 1 (off)

	SPI_write(0xFF);						// extra clocks to allow SD to finish off what it is doing

	digitalwrite(SPI_SLAVE_SELECT, LOW);	// set SS = 0 (on)

	SPI_write(0x50);						// send SD command one to bring out of idle state
	SPI_write(0x00);
	SPI_write(0x00);
	...
	digitalwrite(SPI_SLAVE_SELECT, HIGH);	// set SS = 1 (off)
	//puts("Got set block length response from SD\n\r");
	return 0;
}

/************************** SD Get Status **************************************/
/* Get the status register of the SD, for debugging purposes */

int SD_get_status()
{
	digitalwrite(SPI_SLAVE_SELECT, LOW);	// set SS = 0 (on)

	SPI_write(0x58);						// send SD command one to bring out of idle state
	SPI_write(0x00);
	SPI_write(0x00);
	SPI_write(0x00);
...
int SD_write_block(u16 block_number)
{
	...
	//puts("Write block\n\r");				// block size has been set in SD_init()

	digitalwrite(SPI_SLAVE_SELECT, LOW);	// set SS = 0 (on)

	SPI_write(0x58);						// send SD write block
	SPI_write(HIGH(varh));
	SPI_write(LOW(varh));
	SPI_write(HIGH(varl));
	SPI_write(0x00);						// always zero as mulitples of 512
...
/************************** SD Read Block **************************************/
/**** Reads a 512 Byte block from the SD and out//puts each byte to RS232 ****/
...
int SD_read_block(u16 block_number)
{
	...
	SPI_write(0x51);						// send SD read single block command
	...
	if((SD_response(0x00))==1) return 1;	// if SD_response returns 1 then we failed to get a 0x00 response (affirmative)

	//puts("Got response to read block command\n\r");

	if((SD_response(0xFE))==1) return 1;	// wait for data token
	...
	digitalwrite(SPI_SLAVE_SELECT, HIGH);	// set SS = 1 (off)
	SPI_write(0xFF);						// give SD the clocks it needs to finish off
...
/************************** SD get response **************************************/
/**** Repeatedly reads the SD until we get the response we want or timeout ****/
