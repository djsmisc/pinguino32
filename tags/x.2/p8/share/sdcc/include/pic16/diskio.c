...
/** I/O pin definitions ****************************************/
#define SD_CS			2		// Card Chip Select on pin 2 (RB2)
#define SD_CD			3		// Card Detect on pin 3 (RB3)
#define SD_WP			4		// Card Write Protect on pin 4 (RB4)
...
static UINT Time;		/* 1000Hz (1ms) decrement timer */
...
static u8 wait_ready(void)
{
	...
	Time = 500;							/* Wait for ready in timeout of 500ms */
	start_time = Millis();
	
	SPI_write(DUMMY);
	do
		res = SPI_write(DUMMY);
	while ( (res != 0xFF) && ( (Millis() - start_time) < Time) );
...
static void power_on(void)
{
	...
	SPI_setClockDivider(SPI_MASTER_FOSC_64);
	SPI_init();
}
...
DSTATUS disk_initialize(void)
{
	...
	if (digitalread(SD_CD))
		return DRES_ERROR;								/* No card in the socket */
	...
	if (ty)
	{
		...
		//FCLK_FAST();
		SPI_setClockDivider(SPI_MASTER_FOSC_16);
	 	SPI_init();
		return DRES_OK;
	}
	else
