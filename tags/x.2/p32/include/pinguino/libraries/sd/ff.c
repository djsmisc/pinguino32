...
typedef struct {
	...
	PF_WORD idx; /* File ID 3, directory index */
	PF_WORD ctr; /* File open counter, 0:none, 0x01..0xFF:read open count, 0x100:write mode */
}FILESEM;
#endif
...
#define IsDBCS1(c)	(((PF_BYTE)(c) >= _DF1S && (PF_BYTE)(c) <= _DF1E) || ((PF_BYTE)(c) >= _DF2S && (PF_BYTE)(c) <= _DF2E))
#else			/* One 1st byte area */
#define IsDBCS1(c)	((PF_BYTE)(c) >= _DF1S && (PF_BYTE)(c) <= _DF1E)
#endif

#ifdef _DS3S	/* Three 2nd byte areas */
#define IsDBCS2(c)	(((PF_BYTE)(c) >= _DS1S && (PF_BYTE)(c) <= _DS1E) || ((PF_BYTE)(c) >= _DS2S && (PF_BYTE)(c) <= _DS2E) || ((PF_BYTE)(c) >= _DS3S && (PF_BYTE)(c) <= _DS3E))
#else			/* Two 2nd byte areas */
#define IsDBCS2(c)	(((PF_BYTE)(c) >= _DS1S && (PF_BYTE)(c) <= _DS1E) || ((PF_BYTE)(c) >= _DS2S && (PF_BYTE)(c) <= _DS2E))
#endif
...
static PF_WORD Fsid; /* File system mount ID */

#if _FS_RPATH
static
PF_BYTE CurrVol; /* Current drive */
#endif
...
#define	DEF_NAMEBUF			PF_BYTE sfn[12]
...
#define	DEF_NAMEBUF			PF_BYTE sfn[12]; WCHAR lbuf[_MAX_LFN+1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()

#elif _USE_LFN == 3 		/* LFN feature with dynamic working buffer on the heap */
#define	DEF_NAMEBUF			PF_BYTE sfn[12]; WCHAR *lfn
...
void mem_cpy(void* dst, const void* src, UINT cnt) {
	PF_BYTE *d = (PF_BYTE*) dst;
	const PF_BYTE *s = (const PF_BYTE*) src;
...
int mem_cmp(const void* dst, const void* src, UINT cnt) {
	const PF_BYTE *d = (const PF_BYTE *) dst, *s = (const PF_BYTE *) src;
	int r = 0;
...
PF_WORD idx /* Directory index number */
) {
	DWORD clst;
	PF_WORD ic;
...
#if _USE_LFN
static
const PF_BYTE LfnOfs[] = {1,3,5,7,9,14,16,18,20,22,24,28,30}; /* Offset of LFN chars in the directory entry */

static
int cmp_lfn ( /* 1:Matched, 0:Not matched */
		WCHAR *lfnbuf, /* Pointer to the LFN to be compared */
		PF_BYTE *dir /* Pointer to the directory entry containing a part of LFN */
)
{
	UINT i, s;
	WCHAR wc, uc;
...
int pick_lfn ( /* 1:Succeeded, 0:Buffer overflow */
		...
		PF_BYTE *dir /* Pointer to the directory entry */
)
{
	UINT i, s;
	WCHAR wc, uc;
...
#if _USE_LFN
void gen_numname (
		PF_BYTE *dst, /* Pointer to generated SFN */
		const PF_BYTE *src, /* Pointer to source SFN to be modified */
		const WCHAR *lfn, /* Pointer to LFN */
		PF_WORD seq /* Sequence number */
)
{
	PF_BYTE ns[8], c;
	UINT i, j;
	...
	if (seq > 5) { /* On many collisions, generate a hash number instead of sequential number */
		do seq = (seq >> 1) + (seq << 15) + (PF_WORD)*lfn++; while (*lfn);
	}
...
static
PF_BYTE sum_sfn (
		const PF_BYTE *dir /* Ptr to directory entry */
)
{
	PF_BYTE sum = 0;
	UINT n = 11;
	...
	if (res == FR_OK && ne > 1) { /* Initialize LFN entry if needed */
		...
		if (res == FR_OK) {
			...
			do { /* Store LFN entries in bottom first */
				res = move_window(dj->fs, dj->sect);
				if (res != FR_OK) break;
				fit_lfn(dj->lfn, dj->dir, (PF_BYTE)ne, sum);
				dj->fs->wflag = 1;
				res = dir_next(dj, 0); /* Next entry */
			}while (res == FR_OK && --ne);
		}
...
DIR *dj /* Directory object pointing the entry to be removed */
	...
	res = dir_sdi(dj, (PF_WORD)((dj->lfn_idx == 0xFFFF) ? i : dj->lfn_idx)); /* Goto the SFN or top of the LFN entries */
	if (res == FR_OK) {
		do {
			res = move_window(dj->fs, dj->sect);
			if (res != FR_OK) break;
...
#ifdef _EXCVT
	static const PF_BYTE excvt[] = _EXCVT; /* Upper conversion table for extended chars */
#endif
...
static PF_BYTE check_fs( /* 0:FAT-VBR, 1:Valid BR but not FAT, 2:Not a BR, 3:Disk error */
FATFS *fs, /* File system object */
...
PF_WORD id /* Member id of the target object to be checked */
) {
...
PF_BYTE mode /* Access mode and file open mode flags */
	...
	mode &= FA_READ | FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS
			| FA_CREATE_NEW;
	res = chk_mounted(&path, &dj.fs, (PF_BYTE) (mode & ~FA_READ));
#else
	mode &= FA_READ;
	...
	rbuff += rcnt, fp->fptr += rcnt, *br += rcnt, btr -= rcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
			...
			if (cc) { /* Read maximum contiguous sectors directly */
				...
				if (disk_read(fp->fs->drv, rbuff, sect, (PF_BYTE) cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
	...
	wbuff += wcnt, fp->fptr += wcnt, *bw += wcnt, btw -= wcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
			...
			if (cc) { /* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize) /* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if (disk_write(fp->fs->drv, wbuff, sect, (PF_BYTE) cc)
						!= RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
			...
			if (fp->dsect != sect) { /* Fill sector cache with file data */
				if (fp->fptr < fp->fsize && disk_read(fp->fs->drv, fp->buf,
						sect, 1) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
...
PF_BYTE value, /* Attribute bits */
PF_BYTE mask /* Attribute mask to change */
) {
	FRESULT res;
	DIR dj;
	PF_BYTE *dir;
		...
		if (res == FR_OK) {
			...
			if (!dir) { /* Is it a root directory? */
				...
				dir[DIR_Attr] = (value & mask) | (dir[DIR_Attr]
						& (PF_BYTE) ~mask); /* Apply attribute change */
				dj.fs->wflag = 1;
				res = sync(dj.fs);
			}
		}
...
FRESULT f_forward (
		FIL *fp, /* Pointer to the file object */
		UINT (*func)(const PF_BYTE*,UINT), /* Pointer to the streaming function */
...
)
{
	...
	*bf = 0; /* Initialize byte counter */
	...
	for (; btr && (*func)(0, 0); /* Repeat until all data transferred or stream becomes busy */
		...
		ABORT(fp->fs, FR_DISK_ERR);
		fp->dsect = sect;
		rcnt = SS(fp->fs) - (PF_WORD)(fp->fptr % SS(fp->fs)); /* Forward data from sector window */
		if (rcnt > btr) rcnt = btr;
		rcnt = (*func)(&fp->fs->win[(PF_WORD)fp->fptr % SS(fp->fs)], rcnt);
...
FRESULT f_mkfs (
		PF_BYTE drv, /* Logical drive number */
		PF_BYTE sfd, /* Partitioning rule 0:FDISK, 1:SFD */
		UINT au /* Allocation unit size [bytes] */
)
{
	static const PF_WORD vst[] = {1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 0};
	static const PF_WORD cst[] = {32768, 16384, 8192, 4096, 2048, 16384, 8192, 4096, 2048, 1024, 512};
	PF_BYTE fmt, md, sys, *tbl, pdrv, part;
	...
	if (_MULTI_PARTITION && part) {
		...
		if (sfd) { /* No patition table (SFD) */
			...
			tbl[6] = (PF_BYTE)((n >> 2) | 63); /* Partiiton end sector */
			tbl[7] = (PF_BYTE)n; /* End cylinder */
	...
	tbl[BPB_SecPerClus] = (PF_BYTE)au; /* Sectors per cluster */
...
FRESULT f_fdisk (
		PF_BYTE pdrv, /* Physical drive number */
		const DWORD szt[], /* Pointer to the size table for each partitions */
		void* work /* Pointer to the working buffer */
)
{
	UINT i, n, sz_cyl, tot_cyl, b_cyl, e_cyl, p_cyl;
	PF_BYTE s_hd, e_hd, *p, *buf = (PF_BYTE*)work;
	DSTATUS stat;
	...
	for (i = 0; i < 4; i++, p += SZ_PTE) {
		...
		/* Set partition table */
		p[1] = s_hd; /* Start head */
		p[2] = (PF_BYTE)((b_cyl >> 2) + 1); /* Start sector */
		p[3] = (PF_BYTE)b_cyl; /* Start cylinder */
		p[4] = 0x06; /* System type (temporary setting) */
		p[5] = e_hd; /* End head */
		p[6] = (PF_BYTE)((e_cyl >> 2) + 63); /* End sector */
		p[7] = (PF_BYTE)e_cyl; /* End cylinder */
...
)
{
	...
	if (c < 0x80) { /* 7-bit */
		...
		if (c < 0x800) { /* 11-bit */
			s[0] = (PF_BYTE)(0xC0 | (c >> 6));
			s[1] = (PF_BYTE)(0x80 | (c & 0x3F));
			btw = 2;
		} else { /* 16-bit */
			s[0] = (PF_BYTE)(0xE0 | (c >> 12));
			s[1] = (PF_BYTE)(0x80 | ((c >> 6) & 0x3F));
			s[2] = (PF_BYTE)(0x80 | (c & 0x3F));
			btw = 3;
		}
	}
