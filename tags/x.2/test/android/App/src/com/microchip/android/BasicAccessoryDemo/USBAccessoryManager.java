			...
			if( accessory != null) {			
				...
				if(deviceManager.hasPermission(accessory)) {	
					...
					if(parcelFileDescriptor != null) {
						...
						handler.obtainMessage(what, new USBAccessoryManagerMessage(USBAccessoryManagerMessage.MessageType.READY, accessory)).sendToTarget();
    	...
    	public void run() {
    		...
    		while(continueRunning) {
				try {                
					...
					handler.obtainMessage(what, bytes, -1, new USBAccessoryManagerMessage(USBAccessoryManagerMessage.MessageType.READ, data)).sendToTarget();            
				} catch (IOException e) {       
					//Exiting read thread
					break;            
				}
