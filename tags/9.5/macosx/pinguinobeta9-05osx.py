...
# ------------------------------------------------------------------------------
    ...
    def __init__(self, parent, id=-1, title='Pinguino',
        ...
        self.NEW = wx.MenuItem(self.file_menu, wx.ID_NEW, "New", "", wx.ITEM_NORMAL)
        self.file_menu.AppendItem(self.NEW)
        self.OPEN = wx.MenuItem(self.file_menu, wx.ID_OPEN, "Open", "", wx.ITEM_NORMAL)
        self.file_menu.AppendItem(self.OPEN)
        self.SAVE = wx.MenuItem(self.file_menu, wx.ID_SAVE, "Save", "", wx.ITEM_NORMAL)
        self.file_menu.AppendItem(self.SAVE)
        self.SAVEAS = wx.MenuItem(self.file_menu, wx.ID_SAVEAS, "Save as", "", wx.ITEM_NORMAL)
        self.file_menu.AppendItem(self.SAVEAS)
        self.CLOSE = wx.MenuItem(self.file_menu, wx.ID_CLOSE, "Close", "", wx.ITEM_NORMAL)
        self.file_menu.AppendItem(self.CLOSE)
        self.EXIT = wx.MenuItem(self.file_menu, wx.ID_EXIT, "Exit", "", wx.ITEM_NORMAL)
        ...
        self.UNDO = wx.MenuItem(self.edit_menu, wx.ID_UNDO, "Undo", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.UNDO)
        self.REDO = wx.MenuItem(self.edit_menu, wx.ID_REDO, "Redo", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.REDO)
        self.edit_menu.AppendSeparator()
        self.FIND = wx.MenuItem(self.edit_menu, wx.ID_FIND, "Find", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.FIND)        
        self.REPLACE = wx.MenuItem(self.edit_menu, wx.ID_REPLACE, "Replace", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.REPLACE)
        self.REPLACE.Enable(False)
        self.edit_menu.AppendSeparator()        
        self.CUT = wx.MenuItem(self.edit_menu, wx.ID_CUT, "Cut", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.CUT)
        self.COPY = wx.MenuItem(self.edit_menu, wx.ID_COPY, "Copy", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.COPY)
        self.PASTE = wx.MenuItem(self.edit_menu, wx.ID_PASTE, "Paste", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.PASTE)  
        self.CLEAR = wx.MenuItem(self.edit_menu, wx.ID_CLEAR, "Clear", "", wx.ITEM_NORMAL)
        self.edit_menu.AppendItem(self.CLEAR)
        self.SELECTALL = wx.MenuItem(self.edit_menu, wx.ID_SELECTALL, "Select all", "", wx.ITEM_NORMAL)
        ...
        for th in self.themeList:
            ...
            i = i + 1
        self.pref_menu.AppendMenu(self.ID_THEME, "Themes", self.theme_menu)
        # ---debug submenu
        self.debug_menu = wx.Menu()
        self.USBBULK = wx.MenuItem(self.debug_menu, self.ID_USBBULK, "USB Bulk", "", wx.ITEM_CHECK)
        self.debug_menu.AppendItem(self.USBBULK)
        self.USBBULK.Enable(False)
        self.CDC = wx.MenuItem(self.debug_menu, self.ID_CDC, "CDC", "", wx.ITEM_CHECK)
        self.debug_menu.AppendItem(self.CDC)
        self.CDC.Enable(False)
        self.pref_menu.AppendMenu(self.ID_DEBUG, "Debug", self.debug_menu)
        self.menu.Append(self.pref_menu, "Preferences")
        
                        
        self.HELP = wx.Menu()
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_WEBSITE, "Website", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_BLOG, "Blog", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_FORUM, "Forum", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_GROUP, "Group", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)                                      
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_WIKI, "Wiki", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT) 
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_TUTORIAL, "Tutorial", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)
        self.ABOUT = wx.MenuItem(self.HELP, self.ID_ABOUT, "About...", "", wx.ITEM_NORMAL)
        self.HELP.AppendItem(self.ABOUT)                                                
        ...
        PaneOutputInfo.Resizable(True)
        PaneOutputInfo.Caption("Output")
        ...
        self.PaneEditorInfo.Caption("Editor")
...
# ------------------------------------------------------------------------------
        ...
        self.displaymsg("Welcome to " + pinguino_version + "\n", 0);
...
# ------------------------------------------------------------------------------
    ...
    def OnUpload(self, event):
        if self.editor.GetPath()!=-1:
            ...
            if os.path.exists(filename+".hex"):
                ...
                dlg = wx.MessageDialog(self,
                        'File must be verified before upload','Warning!',
                        wx.OK | wx.ICON_WARNING
                        )
                result=dlg.ShowModal()
                dlg.Destroy()                     
            ...
            dlg = wx.MessageDialog(self,
                    'File must be saved before upload','Warning!',
                    wx.OK | wx.ICON_WARNING
                    )
            result=dlg.ShowModal()
            dlg.Destroy()           
    ...
    def OnFind(self,event):
        ...
        self.PaneFindInfo.Caption("Find")
...
# ------------------------------------------------------------------------------
    ...
    def StartResizeWindow(self,event):
        mouse=wx.GetMouseState()
        lasty=mouse.GetY()
        while (mouse.LeftDown()==True):
            mouse=wx.GetMouseState()
            deltay=mouse.GetY()-lasty
            self.logwindow.SetSize((392, 100)) #self.logwindow.GetMinHeight()+deltay))
            self.editor.SetSize((392,200))
            self.logwindow.Fit()
            self.editor.Fit()             
            self.Fit()
            event.Skip()
    ...
    def getCodeSize(self, filename):
        ...
        fichier.close()
        self.displaymsg("\n" + "code size: " + str(codesize) + " / " + str(totalspace) + " bytes" + " (" + str(100*codesize/totalspace) + "% used)", 0)
